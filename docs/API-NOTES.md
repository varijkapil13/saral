...
| schema | The `/bulk/**` endpoints refuse in a shape of their own: `{"errors":[{"message":"…"}]}`, an **array of objects** where every other endpoint puts either sentences or a field-keyed object. `BulkOperationErrorResponse` is declared for their **400 and 401 only** — 403 is not documented for either endpoint although both are gated on the global Bulk Change permission, so which envelope a 403 arrives in is unknown | `parseErrorBody` reads none of it — an array under `errors` carries nothing — so a documented 400 from a bulk endpoint arrives correctly classified and with no sentence in it, and the user gets this client's fallback wording instead of the site's. Reading it is a `client.go` change, not an adapter one; `pkg/jira/cloud/bulkmove_test.go` fails on it rather than working around it. |
...
| schema | An issue's **sprint value reaches a caller in two shapes**, decided by whether the read expanded `schema`: with a schema the field is an `array` of `items: json`, which no decoder here claims, so the value arrives as `jira.KindUnknown` carrying the JSON; with no schema the same array infers as options, an id and a name. A field list naming any non-system field is what makes a search send the expand, so a timeline gets the first shape | Anything reading a sprint off an issue reads both, or it works in a list view and not in a timeline. Neither shape carries a date, so the dates still come from `GET /sprint/{id}`. |
