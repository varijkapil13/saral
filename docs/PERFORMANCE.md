...
| Frame allocations at 200×60 | ceilings in `internal/ui/kernel/budget_test.go` | 297 for a frame, 310 for a keystroke and its frame, 324 with the mouse on, 1120 for a frame with the destinations up behind a latched `g`, each held to a ceiling about a tenth above. The last is not a steady state — nothing repaints it until the next key — and it is in the table so that what an overlay costs here is written down: the `?` overlay is 776 and the right-click menu 800, both unbudgeted |
