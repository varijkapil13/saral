...
| An overlay over the same frame | ceilings in the same table | 629 for the `?` overlay, 800 for the right-click menu, 1120 for the destinations behind a latched `g`. None is a steady state — nothing repaints one until the next key — and each has a benchmark of its own, so a number here is one the table checks |
