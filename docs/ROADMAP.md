...
- [ ] **P1.1 — Transport and auth** · **owns** `pkg/jira/cloud/{client,auth,retry,paginate}.go`
  Basic auth, `Retry-After` backoff, both paginators, repeated-cursor guard, request coalescing.
- [ ] **P1.2 — Search and JQL** · **owns** `pkg/jira/cloud/search.go`, `internal/app/search.go`
  `POST /search/jql` with explicit field sets, `approximate-count`, saved queries.
- [ ] **P1.3 — Capability probe** · **owns** `pkg/jira/cloud/caps.go`
...
- [ ] **P1.4 — ADF to markdown** · **owns** `pkg/adf/render*.go`
...
- [ ] **P4.1 — List and download** · **owns** `pkg/jira/cloud/attachment.go`
  Ranged download with progress and resume, temp-file-then-rename.
- [ ] **P4.2 — Upload** · same file, second PR (sequential with P4.1)
  Multipart part named `file`, `X-Atlassian-Token: no-check`, multi-file, delete.
- [ ] **P4.3 — Preview** · **owns** `internal/ui/attach/**`
...
- [ ] **P5.1 — Versions** · **owns** `pkg/jira/cloud/version.go`, `internal/ui/release/list*.go`
  CRUD, archive, unresolved counts, bulk fix-version assignment from the list.
- [ ] **P5.2 — The release flow** · **owns** `internal/ui/release/flow*.go`
...
- [ ] **P6.1 — Board configuration** · **owns** `pkg/jira/cloud/board.go`
  Columns by `statusCategory`, estimation field and rank field read from board config — never guessed.
- [ ] **P6.2 — Sprint lifecycle** · **owns** `pkg/jira/cloud/sprint.go`
...
- [ ] **P6.3 — Board and backlog views** · **owns** `internal/ui/board/**`, `internal/ui/backlog/**`
...
- [ ] **P7.1 — Move wizard** · **owns** `pkg/jira/cloud/bulkmove.go`, `internal/ui/move/**`
...
- [ ] **P8.1 — Date resolution** · **owns** `internal/app/dates.go`
  The cascade that gives every issue a start and an end (see below). Reports provenance per bar.
- [ ] **P8.2 — Timeline view** · **owns** `internal/ui/timeline/**`
...
- [ ] **P8.3 — Plans** · **owns** `pkg/jira/cloud/plan.go`, `internal/ui/plan/**`
...
- [ ] **P9.1 — Release engineering** — goreleaser dry-run, Homebrew tap, install script, `v0.1.0`.
- [ ] **P9.2 — Performance gate** — benchmarks in CI with `benchstat` regression detection.
- [ ] **P9.3 — README, demo GIF, docs pass.**
