...
func TestIssuePatch_IsEmptyOnlyWhenItWouldChangeNothing(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "a label added", patch: jira.IssuePatch{AddLabels: []string{"l"}}},
		{name: "a fix version added", patch: jira.IssuePatch{AddFixVersions: []string{"10"}}},
		{name: "a fix version removed", patch: jira.IssuePatch{RemoveFixVersions: []string{"10"}}},
