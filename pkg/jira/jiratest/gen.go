...
// VersionsFor is the version set the fake seeds a project with, in the order it
// seeds them. A test names one through this rather than writing an id down: the
// ids are numbers, which is the only shape a site mints, and a test that spells
// one out is a test that has to be edited when they change.
func VersionsFor(projectKey string) []jira.Version { return fakeVersionsFor(projectKey) }
...
func fakeVersionsFor(projectKey string) []jira.Version {
	...
	return []jira.Version{
		{
			ID: fakeVersionID(projectKey, 0), ProjectID: pid, Name: "1.0",
		...
		{
			ID: fakeVersionID(projectKey, 1), ProjectID: pid, Name: "2.0",
		...
		{
			ID: fakeVersionID(projectKey, 2), ProjectID: pid, Name: "3.0",
			Description: "Not planned yet",
		},
	}
}

// fakeVersionID is a seeded version's id, derived from the project key so that
// Gen and WithProject agree on it. It is a number because that is the only shape
// a site mints: cloud.ReleaseVersion refuses anything else, since the sweep names
// the version in JQL where a non-numeric fixVersion matches version names.
func fakeVersionID(projectKey string, at int) string {
	return strconv.Itoa(50000 + int(fakeHash32("version:"+strings.ToUpper(projectKey))%900)*10 + at)
}
