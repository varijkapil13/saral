...
func comment(t *testing.T, f *jiratest.Fake, key string, paragraphs ...string) {
	...
	if _, err := f.AddComment(t.Context(), key, adf.NewDoc(nodes...)); err != nil {
		t.Fatalf("AddComment: %v", err)
	}
}

// seedOf is the row the list would have handed over: identity and the few
// fields a list projection carries, and nothing else.
func seedOf(t *testing.T, f *jiratest.Fake, key string) jira.Issue {
	t.Helper()
	...
	return jira.Issue{
		ID: full.ID, Key: full.Key, Summary: full.Summary,
		Type: full.Type, Status: full.Status, Assignee: full.Assignee, Updated: full.Updated,
	}
}
...
func newDriver(t *testing.T, d kernel.Deps, seed jira.Issue, w, h int) *driver {
	t.Helper()
	view, ok := New(d, seed).(*Model)
	if !ok {
...
func golden(t *testing.T, name, got string) {
	...
	if err != nil {
		t.Fatalf("%v — run: go test ./internal/ui/issue -update", err)
	}
	if string(want) != got {
