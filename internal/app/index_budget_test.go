...
// tag on its own would mean they never ran anywhere, so ci.yml has a lane
// without the detector that runs exactly these.
//
// None of them may be parallel. An allocation count comes from process-wide
// MemStats, so a benchmark run beside another test is handed that test's
// allocations divided by its own iteration count.

func TestBudget_IndexSearchAtTenThousandIssues(t *testing.T) {
...
func TestBudget_IndexSearchAllocatesOnlyTheAnswerItHandsBack(t *testing.T) {
...
func TestBudget_IndexRebuildAtTenThousandIssues(t *testing.T) {
